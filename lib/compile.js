...
/** @typedef {{ title?: string, subchapterTitle?: string, synopsis?: string, content?: string }} Doc */

/** @param {TreeFolder[]} tree @param {Record<string, Doc>} docs */
export function compileMarkdownDownload(tree, docs) {
  let md = '';
  ...
  saveAs(new Blob([md], { type: 'text/markdown;charset=utf-8' }), `manuscript-${Date.now()}.md`);
}

/** @param {TreeFolder[]} tree @param {Record<string, Doc>} docs */
export async function compileDocxDownload(tree, docs) {
  const children = [];
  ...
  saveAs(blob, `manuscript-${Date.now()}.docx`);
}
